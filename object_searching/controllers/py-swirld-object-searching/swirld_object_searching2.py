...
class Node:
    ...
    def smart_contract(self,robot_id,x):
        ...
        if x and x!=-2:
            ...
            if robot_id in groups[0]:
                self.calculate_group_mean(x,0)
            elif robot_id in groups[1]:
                self.calculate_group_mean(x,1)
            elif robot_id in groups[2]:
                self.calculate_group_mean(x,2)
            elif robot_id in groups[3]:
    ...
    def calculate_group_mean(self,x,group_id):
        if x not in self.y[group_id] and not self.consensusReached[group_id]:
            self.y[group_id].append(x)
            
        if len(self.y[group_id]) == 1:
    ...
    def main(self):
        ...
        while True:
            ...
            if self.stopsend == False and self.allconsensusReached == True:
                ...
                #f2.close()
                respond = "#"+str((self.allconsensusReached,self.y,self.time,hg_size,hg_height))+"~"
