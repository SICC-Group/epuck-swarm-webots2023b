...
class MyRunner(BaseRunner):
    def __init__(self, config):
        ...
        print("\n Env-{}, Algo-{}, Exp-{}, runs total num timesteps-{}/{}. \n".format(
            self.env_name, self.algorithm_name, self.args.experiment_name,
    ...
    def collect_rollout(self, phase="train"):
        ...
        env.reset()
        local_step = 0
        init_actions = [0] * self.num_agents  # stop
        state, reward, done, info = env.step(init_actions)
        ...
        while local_step < self.episode_length and not rospy.is_shutdown():
            ...
            s_, r_, done_, info_ = env.step(self.actions)
            # import pdb; pdb.set_trace()
            episode_states.append(s_)
            episode_actions.append(self.actions)
        ...
        for i in range(self.num_agents):
            env_info[f'episode_r_{i}'] = np.sum(np.array(episode_rewards)[1:, i])
