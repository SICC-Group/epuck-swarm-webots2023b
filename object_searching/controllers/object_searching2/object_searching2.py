...
center = [[0.8,0.8],[-0.8,0.8],[-0.8,-0.8],[0.8,-0.8]]
...
def ObstacleAvoidance():
    ...
    for i in range(group_number):
        if robot_id in groups[i]:
            if self_pos[1]< ROM[i][0] or self_pos[1] > ROM[i][1] or self_pos[0] < ROM[i][2] or self_pos[0] > ROM[i][3]:
                escaped = 1
                arrived = 0                
            else:
                if self_pos[1]> ROM2[i][0] and self_pos[1] < ROM2[i][1] and self_pos[0] > ROM2[i][2] and self_pos[0] < ROM2[i][3]:
                    arrived = 1
                    escaped = 0
                    if doMoveBack:                        
                        doRandomMove = 1
    ...
    if escaped:
        ...
        print(self_pos)
        print(self_pos[1]< ROM[i][0] or self_pos[1] > ROM[i][1] or self_pos[0] < ROM[i][2] or self_pos[0] > ROM[i][3])
        if not doMoveBack:
...
def DetectCell():
    ...
    if flag:
        ...
        x = self_pos[1] - 0.03*math.sin(self_rot)  
        z = self_pos[0] - 0.03*math.cos(self_rot)   
