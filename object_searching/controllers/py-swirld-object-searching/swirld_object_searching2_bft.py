...
class Node:
    def __init__(self, node_id, kp, network, n_nodes, stake):
        ...
        self.validated = set()
        self.confirmed = set()
        self.reputation = {i: 0 for i in range(N)}
        self.proposer = {}
        self.validator = {}
        self.stop = False
        self.stopsend = False
    ...
    def extract_data(self, r_id, x):
        if x and x != -2:

            if x not in self.candidate:
                self.candidate.add(x)
                self.proposer[x] = r_id
                self.validator[x] = set()
            else:
                if r_id != self.proposer[x]:
                    self.validated.add(x)
                    self.validator[x].add(r_id)
            if len(self.validated) == 4:
                self.consensusReached = True
        elif x == -2:
            self.stop = True
        else:
            logger.info("Data is not defined")
        if self.consensusReached:
            self.allconsensusReached = True
            print("self.allconsensusReached:" + str(self.allconsensusReached))
    ...
    def main(self):
        ...
        while True:
            ...
            if payload:
                ...
                if neighbor_pks:
                    ...
                    if new == ():
                        ...
                        t2 = time()
                        if self.id == 9:
                            print("time_for_consensus: %6f" % (t2 - t1))
                    self.pre_payload = []
            ...
            if self.stopsend == False and self.allconsensusReached == True:
                ...
                # f2 = open("/home/syc/hashgraph.txt", "w")
                # f2.write(str(self.hg))
                # f2.close()
                respond = "#" + str((self.allconsensusReached, self.validated, self.time, hg_size, hg_height)) + "~"
                print(str(self.id) + respond)
                print(self.validated)
                agent.send_data(respond.encode())
                self.stopsend = True
